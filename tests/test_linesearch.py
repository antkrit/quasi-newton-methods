...
def test_backtracking_using_derivative(x_k, p_k):
    """Test function output, if a derivative function is provided."""
    _, fval, old_fval = backtracking(x_k, p_k, parabola, parabola_derivative)
    assert np.all(fval < old_fval)
...
def test_backtracking_without_derivative(x_k, p_k):
    """Test function output, if a derivative function is omitted."""
